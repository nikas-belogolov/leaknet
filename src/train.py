...
def fit_model(model: BaseModel, train_loader, val_loader, epochs=EPOCHS, show_progress_bar=True):
    ...
    for epoch in range(epochs):
        ...
        if show_progress_bar:
            pbar = tqdm(total=len(train_loader), desc=f"Epoch {epoch + 1}/{epochs}", unit=" batches")
        else:
            pbar = None  # Don't use progress bar if show_progress is False
        ...
        if pbar: pbar.set_postfix(metrics)
        ...
        if metrics['val_loss'] < best_val_loss:
            ...
            checkpoints_path = path.join(CHECKPOINTS_DIR, type(model).__name__ + ".pth")
